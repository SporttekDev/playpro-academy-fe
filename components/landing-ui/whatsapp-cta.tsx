// components/landing-ui/whatsapp-cta.tsx
"use client"
...
const CTA_LABEL = "Daftar Trial Class Sekarang!"
const DISMISS_KEY = "wa-cta-dismissed"
...
export default function WhatsAppCTA() {
    ...
    useEffect(() => {
        ...
        if (!dismissed) setShowLabel(true)
    }, [])

    function handleDismissLabel(e: React.MouseEvent) {
        e.preventDefault()
        e.stopPropagation()
        setShowLabel(false)
    ...
    return (
        <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-2 md:bottom-8 md:right-8">
            <AnimatePresence>
                {showLabel && (
                    <motion.div
                        ...
                        className="relative flex items-center gap-2 rounded-full bg-white px-4 py-2.5 shadow-[0_8px_24px_rgba(15,23,42,0.15)]"
                    >
            ...
            <motion.a
                ...
                whileTap={{ scale: 0.95 }}
                className="relative flex h-14 w-14 items-center justify-center rounded-full bg-[#25D366] text-white shadow-[0_8px_24px_rgba(37,211,102,0.4)] transition-shadow hover:shadow-[0_8px_32px_rgba(37,211,102,0.6)]"
